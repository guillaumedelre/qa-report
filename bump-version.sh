...
cp qat.phar downloads/qat-${TAG}.phar
git add downloads/qat-${TAG}.phar

SHA1=$(openssl sha1 qat.phar)

JSON='name:"qat.phar"'
JSON="${JSON},sha1:\"${SHA1}\""
JSON="${JSON},url:\"http://guillaumedelre.github.io/qat/downloads/qat-${TAG}.phar\""
JSON="${JSON},version:\"${TAG}\""

if [ -f qat.phar.pubkey ]; then
